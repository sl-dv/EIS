...
void Graph::read_from_file(const std::string& filename) {
    if (not filename.ends_with(".standardized")){
        std::cout << "WARNING: Make sure the input file is a zero-indexed edge list of a simple undirected graph."<<std::endl;
    }
    ...
    while (std::getline(infile, line)) {
        ...
        if (line[0] == 'n'){ //An optional line "n 000" for the number of nodes
        ...
        addEdge(u, v, incremental);
    }

    infile.close();

    // Sort the adjacency list for each node to enable fast lookups
    for (auto& neighbors : _adjList) {
        std::sort(neighbors.begin(), neighbors.end());
        neighbors.erase(std::unique(neighbors.begin(), neighbors.end()), neighbors.end()); // Remove duplicates
